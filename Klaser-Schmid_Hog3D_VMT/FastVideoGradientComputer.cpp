...
//	_tmpFrame = IplImageWrapper(size, IPL_DEPTH_8U, _video->getFrame()->nChannels);
    ...
    {
        _tmpFrame = IplImageWrapper(size, IPL_DEPTH_16U, _vmt->getFrame()->nChannels);
    }
    else
    {
        _tmpFrame = IplImageWrapper(size, IPL_DEPTH_8U, _vmt->getFrame()->nChannels);
    }
...
//	return static_cast<std::size_t>(_video->numOfFrames());
    return static_cast<std::size_t>(_vmt->numOfFrames());
}
...
//	return static_cast<int>(round(_video->getWidth() * _scaleFactor));
    return static_cast<int>(round(_vmt->getWidth() * _scaleFactor));
}

int FastVideoGradientComputer::getHeight() const
{
//	return static_cast<int>(round(_video->getHeight() * _scaleFactor));
    return static_cast<int>(round(_vmt->getHeight() * _scaleFactor));
}
