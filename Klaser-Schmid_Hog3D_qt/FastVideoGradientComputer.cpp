...
#include <opencv/imagesequence.h>
...
//		if (frame < 0 || frame >= _video->numOfFrames())
        if (frame < 0 || frame >= _imgSequence->numOfFrames())
			return false;
...
//	return static_cast<std::size_t>(_video->numOfFrames());
    return static_cast<std::size_t>(_imgSequence->numOfFrames());
}
...
//	return static_cast<int>(round(_video->getWidth() * _scaleFactor));
    return static_cast<int>(round(_imgSequence->getWidth() * _scaleFactor));
}

int FastVideoGradientComputer::getHeight() const
{
//	return static_cast<int>(round(_video->getHeight() * _scaleFactor));
    return static_cast<int>(round(_imgSequence->getHeight() * _scaleFactor));
}
