...
{
    AVFormatContext* _avp = avformat_alloc_context();
	if (_avp == NULL)
...
Stream OutputFormatContext::AddStream(enum AVMediaType type)
{
	return AddStream(type, av_guess_codec(get()->oformat, NULL, get()->filename, NULL, type));
}

Stream OutputFormatContext::AddStream(enum AVMediaType type, enum AVCodecID codec_id)
{
...
{
	return AddStream(AVMEDIA_TYPE_VIDEO);
}

Stream OutputFormatContext::AddVideoStream(const Codec& codec)
{
	return AddStream(AVMEDIA_TYPE_VIDEO, codec.GetID());
}
