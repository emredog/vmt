...
//            Video::FrameIndex iFrame = vm["dump-frame"].as<std::size_t>();
            ImageSequence::FrameIndex iFrame = vm["dump-frame"].as<std::size_t>();
//            if (!video.seek(iFrame))
            if (!imageSeq.seek(iFrame))
...
//            cvSaveImage(filename.c_str(), video.getFrame());
            cvSaveImage(filename.c_str(), imageSeq.getFrame());
            return EXIT_SUCCESS;
        }
...
//                    double nextShot = video->numOfFrames();
                    double nextShot = imgSequence->numOfFrames();
