...
void CodecContext::OpenDecoder()
{
	...
	if (avcodec_open2(get(), const_cast<AVCodec*>(decoder.get()), NULL) < 0) throw(std::runtime_error("Could not open input codec"));
}

void CodecContext::OpenEncoder()
{
	Codec encoder = GetEncoder();
	if (avcodec_open2(get(), const_cast<AVCodec*>(encoder.get()), NULL) < 0) throw(std::runtime_error("Could not open output codec"));
}
...
boost::optional<Frame> CodecContext::DecodeFrame(const Packet& pkt)
{
	...
	if (avcodec_decode_video2(get(), frame.get(), &frame_ready, pkt.get()) < 0)
